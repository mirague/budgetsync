    ...
    const tasks = new Listr([
      {
        ...
        task: async (ctx: Context) => {
          ctx.cryptoHoldingAmount = Math.round(
            (await this.getCryptoValue()) * 1000,
          )
          return true
        },
      },
      {
        ...
        task: async (ctx: Context) => {
          const {
            data: { account },
          } = await ynabAPI.accounts.getAccountById(
            process.env.YNAB_BUDGET_ID!,
            process.env.YNAB_CRYPTO_ACCOUNT_ID!,
          )
          ctx.ynabAccountBalance =
            account.cleared_balance + account.uncleared_balance
          return true
        },
      },
      {
        ...
        skip: (ctx: Context) => {
          ctx.diff = Math.round(
            ctx.cryptoHoldingAmount - ctx.ynabAccountBalance,
          )
          return Math.abs(ctx.diff) < 1000 ? 'Nothing to reconcile' : false
        },
        task: async (ctx: Context) =>
          ynabAPI.transactions.createTransaction(process.env.YNAB_BUDGET_ID!, {
            transaction: {
  ...
  getCryptoValue = async (): Promise<number> => {
    ...
    try {
      ...
      if (isNaN(v)) {
        ...
        throw new Error(
          'Could not load current holding value from Google Sheet',
        )
      }
